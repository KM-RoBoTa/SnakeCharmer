...
int DXLHAL::dxl_hal_set_baud( float baudrate )
{
    struct serial_struct serinfo;
...
void DXLHAL::dxl_hal_clear(void)
{
    tcflush(gSocket_fd, TCIFLUSH);
}

int DXLHAL::dxl_hal_tx( unsigned char *pPacket, int numPacket )
{
    return write(gSocket_fd, pPacket, numPacket);
}

int DXLHAL::dxl_hal_rx( unsigned char *pPacket, int numPacket )
{
...
void DXLHAL::dxl_hal_set_timeout( int NumRcvByte )
{
    glStartTime = myclock();
    gfRcvWaitTime = (float)(gfByteTransTime*(float)NumRcvByte + 5.0f);
}

int DXLHAL::dxl_hal_timeout(void)
{
    long time;
